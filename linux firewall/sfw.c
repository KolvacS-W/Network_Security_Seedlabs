...
// rule for task 1.1: Prevent A from doing `telnet` to Machine B
{
    ...
    if (iph->protocol == IPPROTO_TCP && tcph->dest == htons(23) && eq_daddr(iph, "172.16.133.130") && eq_saddr(iph, "172.16.133.129"))
    {
        printk(KERN_INFO "kill telnet:Dropping telnet from %pI4 packet to %pI4\n", &iph->saddr, &iph->daddr);
        return NF_DROP;
    }
    else
    {
...
// rule for task 1.2: Prevent B from doing `telnet` to Machine A
{
    ...
    if (iph->protocol == IPPROTO_TCP && tcph->dest == htons(23) && eq_daddr(iph, "172.16.133.129") && eq_saddr(iph, "172.16.133.130"))
    {
        printk(KERN_INFO "kill telnet:Dropping telnet from %pI4 packet to %pI4\n", &iph->saddr, &iph->daddr);
        return NF_DROP;
    }
...
// please NOTE that `netfliter` cannot block a domain for that there are usually multiple ip addresses for a domain.
{
    ...
    iph = ip_hdr(skb);
    tcph = (void *)iph + iph->ihl * 4;
    if (tcph->dest == htons(80) || (tcph->dest == htons(443)) && (eq_daddr(iph, "202.120.224.81"))&& (eq_saddr(iph, "172.16.133.129")))
    {
...
int setUpFilter(void)
{
    ...
    FilterHookRule[2] = (struct nf_hook_ops){.hook = block_fudan, .hooknum = NF_INET_LOCAL_OUT, .pf = PF_INET, .priority = NF_IP_PRI_FIRST};

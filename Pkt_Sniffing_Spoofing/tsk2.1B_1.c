...
int main(int argc, char *argv[])
{
	...
	char filter_exp[100]="icmp && host 172.16.133.128 && host 172.16.133.129";
	
	
	bpf_u_int32 net;
	
	handle=pcap_open_live("ens33",BUFSIZ,1,1000,errbuf);   //open a handle

	if(pcap_compile(handle,&fp,filter_exp,1,net)==-1) //set filter
		printf("Filter error");

	pcap_setfilter(handle,&fp); //apply filter

	pcap_loop(handle,-1,handle_pkt,NULL); //receive packets and call print function
